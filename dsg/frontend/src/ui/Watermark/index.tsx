...
import { getPixelRatio, reRendering, toList } from './utils'
...
const Watermark: React.FC<WatermarkProps> = (props) => {
    ...
    const {
        color = 'rgb(0 0 0 / 10%)', // 默认字体颜色为淡黑色。
        fontSize = 16, // 默认字体大小为16像素。
        fontWeight = 400, // 默认字体粗细为正常。
        fontStyle = 'normal', // 默认字体样式为正常。
        fontFamily = `-apple-system, BlinkMacSystemFont, 'Helvetica Neue', 'PingFang SC', 'Microsoft YaHei', 'sans-serif'`, // 默认字体家族为一系列常见无衬线字体。
        textAlign = 'center', // 默认文本对齐方式为居中。
    } = font
    ...
    const childNode = inherit ? (
        <WatermarkContext.Provider value={watermarkContext}>
            {children}
