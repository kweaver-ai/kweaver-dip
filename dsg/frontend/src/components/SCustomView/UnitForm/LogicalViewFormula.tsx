...
import { cnLowercaseEnNumNameReg, DATA_TYPE_MAP, useQuery } from '@/utils'
 ...
 * 输出库表算子配置
 */
const LogicalViewFormula = forwardRef((props: IFormulaConfigEl, ref) => {
    const {
        visible,
        graph,
        node,
        ...
        if (config) {
            ...
            setFieldItems(
                config_fields.map((info) => {
                    let findItem = preOutData.find(
                        (d) =>
                            d.id === info.id && d.sourceId === info?.sourceId,
                    )
                    if (findItem) {
                    ...
                    findItem =
                        preOutData.find((a) => a.alias === info.alias) ||
                        preOutData.find(
                            (a) =>
                                a.alias.toLowerCase() ===
                                info.alias.toLowerCase(),
                        )
                    if (findItem) {
                        if (!info.canChanged) {
                            const err = checkDataType({
            ...
            setFieldItems(
                res.map((info) => {
                    const tempFl = info
                    const findItem =
                        preOutData.find((a) => a.alias === tempFl.alias) ||
                        preOutData.find(
                            (a) =>
                                a.alias.toLowerCase() ===
                                tempFl.alias.toLowerCase(),
                        )
                    if (findItem) {
    ...
    const handleSearchField = (kw: string) => {
        setSearchItems(
            fieldItems?.filter((info) => {
                return info.alias?.toLowerCase().includes(kw.toLowerCase())
                // ||
    ...
    const checkDataType = (item): boolean => {
        const fieldType =
            ...
            fieldsData.data.find((a) => a.id === item?.selectField?.id)
                ?.data_type
        if (fieldType && item.data_type && item.data_type !== fieldType) {
            return true
        }
        return false
    }
