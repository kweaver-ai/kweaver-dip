...
const runFormula = async (
    ...
    return runSceneView(10, 1, params, false, 'data-view')
}
            ...
            if (errorFields.length > 0 || selectedFields.length === 0) {
                ...
                outData = selectedFields.map((a) => {
                    ...
                    if (
                        ...
                        // findItem = preNodes[0]?.data?.output_fields.find(
                        //     (b) => b.id === a.id && b.sourceId === a.sourceId,
                        // )
                        return {
                            ...a,
...
const checkSQLViewFormulaConfig = async (
    ...
    if (!configTables.every((element) => preNodeNames.includes(element))) {
        errorState = true
    }
    ...
    if (!configFields.every((element) => preAliasOutData.includes(element))) {
        errorState = true
    }
...
const SceneGraphProvider = ({ children }: { children: ReactNode }) => {
    ...
    const [citeFormViewId, setCiteFormViewId] = useState<string>('')
