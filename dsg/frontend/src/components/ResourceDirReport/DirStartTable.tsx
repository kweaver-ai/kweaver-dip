...
const DirStartTable = forwardRef((props: IDirTable, ref) => {
    ...
    // 排序
    const [selectedSort, setSelectedSort] = useState<any>(DefaultMenu.waiting)
    ...
    const handleReport = async (params: {
        ...
        try {
            ...
            // 提示
            message.success(getReportMsgConfig(__('上报')))
            refresh()
        } catch (error) {
            formatError(error)
        }
        ...
        switch (op) {
            ...
            case OperateType.REPORT:
                ...
                handleReport({
                    ...
                    operation: ISSZDAuditOperation.Report,
                })
                break
    ...
    const handleTableChange = (sorter) => {
        ...
        if (searchCondition.sort === 'update_time') {
    ...
    const columns: any = [
        ...
        },
        {
            ...
            render: (_: string, record) => {
                ...
                // record.audit_status !== ISSZDReportAuditStatus.Cancel
        ...
        if (allReq?.length) {
            Promise.allSettled(allReq).then((values) => {
                ...
                if (errorLen) {
                    ...
                    message.success(getReportMsgConfig(__('上报'), hasRepeat))
                }
                refresh()
            })
            return
            ...
            <SearchLayout
                ...
                suffixNode={
                    <SortBtn
                        contentNode={
                            <DropDownFilter
                                menus={[DefaultMenu.waiting]}
                                defaultMenu={DefaultMenu.waiting}
