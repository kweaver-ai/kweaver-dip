...
const DataUndsGraph: React.FC<IDataUndsGraph> = ({
    ...
    const { viewMode, catalogData, columnData, setCatalogData, setColumnData } =
        useUndsGraphContext()
    ...
    const getCatalogData = async (data?) => {
        try {
            ...
            //         const temp = info
            //         temp.dataInfo!.choices = entries.map((e: any) => {
    ...
    const getCatalogColumnData = async (data?) => {
        try {
            ...
            setColumnData(
                res?.columns?.map((e: any) => ({
                    id: e.id,
                    column_name: e.technical_name,
                    name_cn: e.business_name,
                    data_format: e.data_type,
                })) || [],
            )
            // if (columns) {
            ...
            //     nodeData.forEach((info) => {
            //         const temp = info
            //         temp.dataInfo!.choices = columns.map((e) => {
    ...
    const assemblyData = (data: MindMapData[]) => {
        return data
            .filter((info) => [NodeType.SecondLeNode].includes(info.nodeType))
            .map((info) => {
    ...
    const flatSubByConfigId = (data: MindMapData[], id: string) => {
        return data
            ...
            .map((info) => {
                if (info.children?.[0].nodeType === NodeType.FourthLeNode) {
                    return {
                        id: info.dataInfo!.configId,
                        children: flatSubByConfigId(
                            data,
    ...
    const publish = async () => {
        ...
        try {
            await updateDataComprehension(catalogId, {
                ...
                catalog_create: originType === 'resourceDir',
            })
            message.success(
                saveType === SaveType.PUBLISH ? __('提交成功') : __('保存成功'),
            )
            onReturn()
        } catch (err) {
