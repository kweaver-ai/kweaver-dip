...
const MulSelectInputNodeComponent = (props: any) => {
    const { viewMode, catalogData } = useUndsGraphContext()
    ...
    useMemo(() => {
        if (viewMode === ViewMode.VIEW || catalogData.length === 0) {
            setSelectOptions(data.dataInfo?.content?.catalog_infos || [])
        } else {
            setSelectOptions(catalogData)
        }
    }, [catalogData, viewMode])
            ...
            >
                ...
                >
                    <ScrollSelect
                        ...
                        searchKey="title"
                        optionsData={selectOptions.map((item) => ({
                            ...item,
                            value: item.id,
                            label: (
                                <div
