...
const SelectInputNodeComponent = (props: any) => {
    const { viewMode, columnData } = useUndsGraphContext()
    ...
    useMemo(() => {
        if (viewMode === ViewMode.VIEW || columnData.length === 0) {
            setSelectOptions(
                data.dataInfo?.content?.column_info
                    ? [data.dataInfo?.content?.column_info]
                    : [],
            ...
            >
                <div ref={selectRef}>
                    <ScrollSelect
                        ...
                        searchKey="label"
                        optionsData={selectOptions.map((info) => ({
                            value: info.id,
                            label: info.name_cn,
                        }))}
