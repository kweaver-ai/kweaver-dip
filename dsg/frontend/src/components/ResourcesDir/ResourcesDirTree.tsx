...
import ResClassifyDirTree from './ResClassifyDirTree'
...
interface IResourcesDirTree {
    ...
    getSelectedNode: (sn: CatlgTreeNode, type?: RescCatlgType) => void
    getCurTabKey: (activeKey: string) => void
    isShowSearch?: boolean
    selectStyle?: CSSProperties
    filterDomainType?: string[] // 允许显示的主题域的层级
    limitDomainTypes?: string[] // 需要屏蔽的主题域的层级
    domainPlaceholder?: string // 主题域搜索的预设文案
    defaultActiveKey?: RescCatlgType // 默认选中的树的tab
    ...
    (props: any, ref) => {
        ...
        const getNodeObjects = async (
            ...
            try {
                ...
                if (oprType === CatlgOperateType.CLKTOEXPAND) {
                    ...
                    // 不展示全部时，默认选中项为第一个节点
                    if (!isShowAll) {
                        getSelectedNode(res.entries?.[0], activeKey)
        ...
        return (
            <div className={styles.treeWrapper}>
                <div className={styles.viewModeWrapper} style={selectStyle}>
                    {selectOptions?.length === 2 ? null : (
                    ...
                    {selectOptions?.length === 1 ? (
                        ...
                        <Radio.Group
                            onChange={(e) => {
                                setActiveKey(e.target.value as RescCatlgType)
                                if (e.target.value !== RescCatlgType.DOAMIN) {
                                    setSelectedId('all')
                                    getSelectedNode(allNodeInfo, e.target.value)
                                }
                            }}
                            value={activeKey}
                        ...
                        <Select
                            value={activeKey}
                            bordered={false}
                            options={selectOptions || rescCatlgItems}
                            onChange={(key) => {
                                setActiveKey(key as RescCatlgType)
                                if (key !== RescCatlgType.DOAMIN) {
                                    setSelectedId('all')
                                    getSelectedNode(allNodeInfo, key)
                                }
                            }}
                ...
                /> */}
                {activeKey === RescCatlgType.DOAMIN ? (
                    <div
                        style={{
                    ...
                    >
                        ...
                        <GlossaryDirTree
                            ...
                            filterType={filterDomainType}
                            limitTypes={limitDomainTypes}
                    ...
                    >
                        <ArchitectureDirTree
                            getSelectedNode={(sd) =>
                                getSelectedNode(sd, RescCatlgType.ORGSTRUC)
                            }
                            type={RescCatlgType.ORGSTRUC}
                            filterType={[
                    ...
                    >
                        <ResClassifyDirTree
                            getSelectedNode={(sd) =>
                                getSelectedNode(sd, RescCatlgType.RESC_CLASSIFY)
                            }
                            isShowAll={isShowAll}
                            isShowSearch={isShowSearch}
                        />
