...
const ViewFieldDetail = ({ onClose, data, node }: ViewFieldDetailType) => {
    ...
    const initFormConfig = async () => {
        ...
        const standardConfig =
            node?.data?.formInfo?.table_kind === FormTableKind.STANDARD
                ? {
        ...
        const config: {
            ...
            group1: {
                type: DisplayInfoComponentType.GroupType,
                label: __('基本信息'),
                expand: true,
                children: {
                    name: {
        ...
        } else if (data.value_range_type === ValueRangeType.Custom) {
            if (config?.group2?.children) {
                config.group2.children.value_range = {
                    type: DisplayInfoComponentType.Text,
                    label: __('自定义'),
    ...
    const getValueRangeTypeDisplay = (valueType, value) => {
        ...
        if (data.value_range_type === ValueRangeType.Custom) {
            return __('自定义')
        }
        return __('无限制')
    }
