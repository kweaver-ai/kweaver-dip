...
import { SortDirection, formatError, getSSZDDemand } from '@/core'
...
import { provinceStatusListOfHandle, provinceStatusListOfToDo } from './const'
...
const DemandMgtList = () => {
    ...
    const [searchCondition, setSearchCondition] = useState<any>({
        ...
        target: tab || DemandProgress.ToDo,
        status: '',
    })
    // 创建表头排序
                    ...
                    <Space size="middle">
                        ...
                        {DemandProgressColunmsConfig[
                            ...
                            <a
                                onClick={() =>
                                    navigate(
                                        `/demand-mgt/province/implement?demandId=${record.id}&demandName=${record.title}`,
                                    )
                                }
                            >
                                {__('提交实施结果')}
        ...
        () => [
            {
                ...
                options:
                    searchCondition.target === DemandProgress.ToDo
                        ? provinceStatusListOfToDo
                        : provinceStatusListOfHandle,
                type: ST.Radio,
            },
        ],
        [searchCondition.target],
    ...
    return (
        <div className={classnames(styles['demand-wrapper'])}>
            <div className={styles.title}>{__('需求处理')}</div>
            <div className={styles['operate-container']}>
                ...
                <Space size={16}>
                    <Space size={8}>
                        ...
                        />
                        {searchCondition.target ===
                        DemandProgress.ToBeSigned ? null : (
                            <LightweightSearch
                                formData={searchData}
                                onChange={(data, key) => {
                                    if (key === 'status') {
