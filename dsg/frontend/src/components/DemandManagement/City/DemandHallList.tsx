...
const DemandHallList = ({ showTitle = true }: IDemandHallList) => {
    ...
    const [activeKey, setActiveKey] = useState<DemandProgress>(deafultTab)

    useEffect(() => {
        if (tab) {
            setActiveKey(tab as DemandProgress)
        } else {
            setActiveKey(
                isDemandHall ? DemandProgress.ToBeSigned : DemandProgress.ToDo,
            )
        }
    ...
    const searchFormData: IFormItem[] = useMemo(() => {
        if (activeKey === DemandProgress.ToBeSigned) {
            return getSearchFormData(statusListOfToBeSigned)
        }
        if (activeKey === DemandProgress.ToDo) {
            return getSearchFormData(statusListOfToDo)
        }
        return getSearchFormData(statusListOfHandle, {
            keyword: defaultKeyword,
        })
    }, [activeKey])
    ...
    const goDetails = (id: string, name: string) => {
        const backUrl = isDemandHall ? BackUrlType.Sign : activeKey
        navigate(
    ...
    // 分析签收
    const analysisSign = async (id: string) => {
        try {
            await analysisSignOffV2(id)
            navigate('/demand-mgt')
        } catch (error) {
            formatError(error)
        }
    ...
    const columns: any = useMemo(() => {
            ...
            },
            {
                ...
                render: (_, record) => (
                    <Space size="middle">
                        ...
                        {DemandProgressColunmsConfig[
                            activeKey
                        ].opertions.includes(DemandOperate.AnalysisSign) &&
                            DemandStatusEnum.AnalysisSigning ===
                                ...
                                <Popconfirm
                                    ...
                                    onConfirm={() => analysisSign(record.id)}
                                    okText={__('确定')}
                                    cancelText={__('取消')}
                                    icon={
                                        <InfoCircleFilled
                        ...
                        {DemandProgressColunmsConfig[
                            activeKey
                        ].opertions.includes(DemandOperate.ImplementSign) &&
        ...
        return cols.filter((col) =>
            DemandProgressColunmsConfig[activeKey].columnKeys.includes(col.key),
        )
    }, [activeKey, tableSort])
        ...
        () => [
            {
                ...
                key: 'status',
                options:
                    activeKey === DemandProgress.ToBeSigned
                        ? statusListOfToBeSigned
                        : activeKey === DemandProgress.ToDo
                        ? statusListOfToDo
                        : statusListOfHandle,
                type: ST.Radio,
            },
            ...
            <div className={styles['operate-container']}>
                ...
                <Space size={16}>
                    <Space size={8}>
                        ...
                        />
                        {activeKey === DemandProgress.ToBeSigned ? null : (
                            <LightweightSearch
                                formData={searchData}
                                onChange={(data, key) => {
                                    if (key === 'status') {
