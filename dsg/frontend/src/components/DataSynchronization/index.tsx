...
import { EditStatus, OptionType, defaultMenu, menus } from './const'
import DataSyncPage from './DataSynsPage'
import EditData from './EditData'
import __ from './locale'
import styles from './styles.module.less'

const DataSynchronization: FC<{
    collapsed: boolean
}> = ({ collapsed }) => {
    // 初始params
    const initialQueryParams: DataSyncParams = {
        offset: 1,
        limit: 10,
    ...
    // 排序
    const [selectedSort, setSelectedSort] = useState<any>(undefined)
    ...
    // 查询params
    const [queryParams, setQueryParams] = useState<DataSyncParams>({
        ...initialQueryParams,
        task_id: taskInfo.id,
    })
    ...
    const getList = async (params) => {
        try {
            setLoading(true)
            const { entries, total_count } = await getSyncDataList(params)
    ...
    const deleteFormData = async (id: string) => {
        try {
            await deleteSyncDataModel(id, { task_id: taskInfo.id })
     ...
     * @param values
     */
    const editSyncDataInfo = async (values) => {
        try {
            await editSyncModelInfo(editData.id, {
    ...
    return (
        <div className={styles.container}>
            {taskInfo.status === TaskExecutableStatus.COMPLETED ? null : (
                <div className={styles.title}>{__('数据同步')}</div>
            )}
            <div
