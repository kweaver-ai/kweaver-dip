...
import { getDataCatalogMountFrontend, getRescDirDetail } from '@/core'

const ResourcesAudit = ({ props }: any) => {
    const {
        props: {
    ...
    const getRescDir = async () => {
        try {
            const res = await getRescDirDetail(id)
            const mountRes = await getDataCatalogMountFrontend(id)
            setDetail({
                ...res,
                mountInfo: mountRes?.mount_resource || [],
            })
    ...
    return (
        <div className={styles.wrapper}>
            ...
            <div className={styles.text}>
                <div className={styles.clums}>{__('目录编号：')}</div>
