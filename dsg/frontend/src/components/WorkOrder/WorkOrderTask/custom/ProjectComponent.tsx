...
export const ProjectLabel: React.FC<{
    ...
    return (
        <div className={styles.projectLabelWrapper} title={label}>
            {label}
        </div>
    )
}

interface IProjectSelect extends SelectProps {
    data: any[]
}
/**
 * 项目选择组件
 * @param data 项目集
 */
export const ProjectSelect: React.FC<IProjectSelect> = ({ data, ...props }) => {
    // 分页
    ...
    const changeOptions = (infos: any[]) => {
        return infos.map((info) => {
            return {
                label: <ProjectLabel label={info.name} />,
                value: info.id,
            }
        })
    }
