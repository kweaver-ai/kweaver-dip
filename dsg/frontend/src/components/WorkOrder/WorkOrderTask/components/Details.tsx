...
import { getOssResourceUrl, stardOrignizeTypeList } from '@/utils'
...
const Details: React.FC<IDetails> = ({ taskId, projectId, getTaskType }) => {
    ...
    const items: TabsProps['items'] = [
        ...
        },
        {
            key: '2',
            label: __('操作记录'),
            children: <OperateRecord taskId={taskId} />,
        },
    ]
    ...
    return (
        <div className={styles.detailWrapper}>
            <div className={styles.left}>
                ...
                {taskDetails.task_type === TaskType.MODEL && (
                    <div className={styles.rowContent}>
                        <div className={styles.label}>{__('关联业务流程')}</div>
                ...
                {(TaskType.DATACOMPREHENSION === taskDetails.task_type ||
                    TaskType.DATACOMPREHENSIONWWORKORDER ===
                        taskDetails.task_type) && (
                    <div className={styles.rowContent}>
                        <div className={styles.label}>{__('所属工单')}</div>
                        <div
                            className={styles.content}
                            title={taskDetails?.work_order_name || '--'}
                            style={{
                                maxWidth: contentWi,
                            }}
                        >
                            <div className={styles.contentMore}>
                                {taskDetails?.work_order_name || '--'}
                            </div>
                        </div>
                    </div>
                )}
                ...
                {TaskType.DATACOMPREHENSIONWWORKORDER ===
                    ...
                    <div className={styles.rowContent}>
                        ...
                        <div
                            className={styles.content}
                            title={
                                taskDetails?.data_comprehension_template_name ||
                                '--'
                            }
                            style={{
                                maxWidth: contentWi,
                        ...
                        <div
                            style={{ width: '80px' }}
                            hidden={
                                !taskDetails?.data_comprehension_template_id
                            }
                        >
                            <a
                                onClick={(e) => {
                                    setCurrentTemplate({
                                        id: taskDetails?.data_comprehension_template_id,
                                    })
                                }}
                            >
                                查看详情
