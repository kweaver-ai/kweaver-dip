...
const JoinTypeSelect: React.FC<{
    disabled?: boolean
    value?: string
    onChange?: (value: string) => void
    joinTypeArr: any[]
}> = ({ disabled, value, onChange, joinTypeArr }) => {
    return (
        <div className={styles.typeWrap}>
            {joinTypeArr.map((info) => (
                <Tooltip key={info} title={joinTypeInfo[info].tip}>
                    <div
                        className={classnames(
                            styles.typeItemWrap,
                            value === info && styles.typeItemSelected,
                    ...
                    >
                        {joinTypeInfo[info].icon}
                        <div className={styles.typeItemTitle}>
                            {joinTypeInfo[info].name}
                        </div>
                        <div className={styles.typeItemCheck}>
                            <CheckOutlined
                                className={styles.typeItemCheckIcon}
                            />
                        </div>
                    </div>
                </Tooltip>
 ...
 * 关联算子配置
 */
const JoinFormula = forwardRef((props: IFormulaConfigEl, ref) => {
    const {
        visible,
        graph,
        node,
    ...
    const handleSave = async () => {
        try {
            ...
            const { relateFieldL, relateFieldR, relation_type } =
                form.getFieldsValue()
    ...
    const checkData = () => {
        ...
        if (config) {
            ...
            form.setFieldValue('relation_type', config.relation_type)
        } else {
            setPreNodes(nodes)
    ...
    const initCheckFieldRepeat = (nodes = preNodes) => {
        let nd = clone(nodes)
        const relateType = form.getFieldValue('relation_type')
        // 右连接重命名左表，其他方式重命名右表
