...
import { FormulaError, formulaInfo, FormulaType } from '../const'
import {
    createNodeInGraph,
    getFormulaErrorText,
    getFormulaMenuItem,
...
const SceneNodeComponent = (props: any) => {
    ...
    const handleAddFormulaItem = async (
        ...
        onRefreshOptionGraphData?.(node, index, item, 'add', type)
    }
    ...
    const handleDeleteOk = (item: IFormula, index: number) => {
        ...
        onRefreshOptionGraphData?.(node, index, item, 'del')
    }
    ...
    const handleEditFormulaItem = (e, item, index) => {
        e.preventDefault()
        e.stopPropagation()
        onOptionGraphData?.({ node, index, item, checkCurrent: false })
    }
