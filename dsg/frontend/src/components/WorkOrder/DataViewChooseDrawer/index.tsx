...
const DataViewChooseDrawer = ({
    ...
    // 选择/取消选择数据资源目录
    const handleChangeForm = (value?: any, isSel?: boolean) => {
        setSelectForm(value)
        if (selectType === 'catlg') {
    ...
    const getItemTooltipTitle = (item: any) => (
        ...
        >
            <div style={{ color: 'rgba(0,0,0,0.45)' }}>{__('库表：')}</div>
    ...
    const getCheckedItemTag = (item: any) => {
        ...
        const name = isCatlg ? item.business_name : item.business_name
        return (
...
export default DataViewChooseDrawer
