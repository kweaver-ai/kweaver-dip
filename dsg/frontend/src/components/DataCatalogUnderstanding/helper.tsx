    ...
    return (
        <div className={styles.undsLabelWrap}>
            <div
                ...
                style={{ background: `${undsStatusInfo[type].color}` }}
            />
            <div className={styles.ul_title}>{undsStatusInfo[type].text}</div>
        </div>
    )
}
...
const createNodeByDataInfo = (
    ...
    return data.map((d) => {
        ...
        const temp = new MindMapData({
            nodeType:
                level === 2 ? NodeType.SecondLeNode : NodeType.ThirdLeNode,
            dataInfo: {
                configId: d.id,
...
const graphRecommendNode = async (
    ...
    const aiData: any[] = await requestRecommendByType(graph, data, nodeId)
    ...
    try {
        ...
        switch (cur.dataInfo.content_type) {
            ...
            case 4:
                return answer
                    .filter((info) =>
                        cur.dataInfo?.choices.find(
                            (c) => c.id === info.column_info?.id,
                        ),
                    )
                    .map((info) => ({
                        ...info,
                        column_info: cur.dataInfo?.choices.find(
                            (c) => c.id === info.column_info?.id,
                        ),
                    }))
                    .filter((info) => {
