...
import { hex2rgba, getActualUrl, getPlatformNumber } from '@/utils'
...
const TaskTypeLabel =
    platform === LoginPlatform.default
        ? {
              ...
              [TaskType.DATACOMPREHENSION]: __('数据目录理解任务'),
              [TaskType.INDICATORPROCESSING]: __('指标开发任务'),
          }
        : {
              //   [TaskType.NORMAL]: __('普通任务'),
              [TaskType.MODEL]: __('业务建模任务'), // 1
              [TaskType.DATAMODELING]: __('数据建模任务'),
              [TaskType.FIELDSTANDARD]: __('字段标准化任务'),
              //   [TaskType.DATASHEETVIEW]: __('同步元数据库表任务'),
              //   [TaskType.DATACOMPREHENSION]: __('数据目录理解任务'),
              //   [TaskType.INDICATORPROCESSING]: __('指标开发任务'),
              [TaskType.DATACOMPREHENSIONWWORKORDER]: __('数据理解工单任务'),
              ...
              [TaskType.MODELINGDIAGNOSIS]: __('业务模型诊断任务'),
              [TaskType.MAINBUSINESS]: __('主干业务梳理任务'),
              [TaskType.STANDARDNEW]: __('标准新建任务'),
          }
...
const TaskTypeColor = {
    ...
    [TaskType.DATAMODELING]: '#126EE3',
    [TaskType.MODELINGDIAGNOSIS]: '#14CEAA',
    [TaskType.MAINBUSINESS]: '#FFA72F',
    [TaskType.STANDARDNEW]: '#8C7BEB',
}

/**
 * 工单类型颜色
 */
const WorkOrderTypeColor = {
    [OrderType.COMPREHENSION]: '#5B91FF',
    [OrderType.FUNSION]: '#3AC4FF',
    [OrderType.STANDARD]: '#8894FF',
    [OrderType.QUALITY_EXAMINE]: '#14CEAA',
    [OrderType.AGGREGATION]: '#FFBA30',
}
...
const getTaskTypeIcon = (
    ...
    switch (true) {
        ...
        case taskType === TaskType.DATASHEETVIEW:
            return (
                <Tooltip
                    title={needToolTip ? TaskTypeLabel[taskType] : ''}
                    placement="bottom"
                >
                    <div
                        ...
                        style={{
                            backgroundColor: hex2rgba(
                                TaskTypeColor[TaskType.DATASHEETVIEW].substring(
                                    1,
                                ),
                                0.1,
                            ),
                    ...
                    >
                        <DatasheetViewOutlined
                            style={{
                                color: TaskTypeColor[TaskType.DATASHEETVIEW],
                                fontSize: 12,
                            }}
                        />
                    </div>
        ...
        case taskType === TaskType.DATAMODELING:
            return (
                <Tooltip
                    title={
                        needToolTip
                ...
                >
                    <div
                        ...
                        style={{
                            backgroundColor: hex2rgba(
                                TaskTypeColor[TaskType.DATAMODELING].substring(
                                    1,
                                ),
                                0.1,
                            ),
        ...
        case taskType === TaskType.MODELINGDIAGNOSIS:
            return (
                <Tooltip
                    title={
                        needToolTip
        ...
        case taskType === TaskType.MAINBUSINESS:
            return (
                <Tooltip
                    title={
                        needToolTip
                ...
                >
                    <div
                        ...
                        style={{
                            backgroundColor: hex2rgba(
                                TaskTypeColor[TaskType.MAINBUSINESS].substring(
                                    1,
                                ),
                                0.1,
                            ),
                    ...
                    >
                        <FontIcon
                            name="icon-yewuliucheng"
                            type={IconType.COLOREDICON}
                            style={{
                                fontSize: 14,
                                color: TaskTypeColor[TaskType.MAINBUSINESS],
                            }}
                        />
                    </div>
                </Tooltip>
            )
        case taskType === TaskType.STANDARDNEW:
            return (
                <Tooltip
                    title={
                        needToolTip
                ...
                >
                    <div
                        ...
                        style={{
                            backgroundColor: hex2rgba(
                                TaskTypeColor[TaskType.STANDARDNEW].substring(
                                    1,
                                ),
                                0.1,
                            ),
                    ...
                    >
                        <FontIcon
                            ...
                            style={{
                                fontSize: 14,
                                color: TaskTypeColor[TaskType.STANDARDNEW],
                            }}
                        />
                    </div>
                </Tooltip>
...
const allTaskTypeList =
    platform === LoginPlatform.default
        ...
        : [
              ...
              TaskType.RESEARCHREPORTWWORKORDER,
              TaskType.DATACATALOGWWORKORDER,
              TaskType.FRONTPROCESSORSWWORKORDER,
          ]

// 游离任务暂时只支持 指标、加工、采集任务
...
/** 业务模型任务子类型列表 */
const modalSubTypeList = [
    {
        value: ModelChildTaskType.FlowChart,
        label: __('录入流程图'),
    },
    {
        value: ModelChildTaskType.NodeForm,
        label: __('录入业务节点表'),
    },
    {
        value: ModelChildTaskType.StandardForm,
        label: __('录入业务标准表'),
    },
    {
        value: ModelChildTaskType.Indicator,
        label: __('录入业务指标'),
    },
    {
        value: ModelChildTaskType.StandardStandardForm,
        label: __('业务标准表标准化'),
    },
]

/** 数据模型任务子类型列表 */
const dataModelSubTypeList = [
    {
        value: ModelChildTaskType.DataSourceForm,
        label: __('录入数据来源表'),
    },
    {
        value: ModelChildTaskType.DataStandardForm,
        label: __('录入数据标准表'),
    },
    {
        value: ModelChildTaskType.DataFusionForm,
        label: __('录入数据融合表'),
    },
    {
        value: ModelChildTaskType.DataIndicator,
        label: __('录入数据指标'),
    },
    {
        value: ModelChildTaskType.DataStandardStandardForm,
        label: __('数据标准表标准化'),
    },
]
