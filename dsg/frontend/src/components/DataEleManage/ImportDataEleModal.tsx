...
interface IImportDataEleModal {
    visible: boolean
    selectedDir?: IDirItem
    update: (newSelectedDir?: IDirItem, newDEList?: Array<IDataElement>) => void
    onClose: () => void
}
 ...
 * @returns
 */
const ImportDataEleModal: React.FC<IImportDataEleModal> = ({
    visible,
    selectedDir,
    update,
    onClose = () => {},
    ...
    // 获取自定义目录
    const getDataEleTreeData = async (
        query?: IDirQueryType,
    ...
    const downloadTempFile = async () => {
        try {
            const res = await exportDataEleBySearch({})
            if (typeof res === 'object' && res.byteLength) {
                streamToFile(res, `数据元导入模板.xlsx`)
    ...
    const handleUpload = async () => {
        ...
        try {
            const res = await importDataEle(catalog_id, formData)
            if (typeof res === 'object' && res.byteLength) {
                streamToFile(
                    res,
                    `数据元_导入失败_${moment(new Date()).format(
                        'YYYYMMDDHHmmss',
                    )}.xlsx`,
                )
    ...
    return (
        <div className={styles.importWrapper}>
            <Modal
                open={visible}
                title={<span className={styles.modalTitle}>导入数据元</span>}
                width={480}
                getContainer={false}
                maskClosable={false}
                destroyOnClose
...
export default ImportDataEleModal
