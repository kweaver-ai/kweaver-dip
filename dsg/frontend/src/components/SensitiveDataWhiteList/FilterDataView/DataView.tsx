...
import { formatError, testWhiteList, getDatasheetViewDetails } from '@/core'
...
const DataView = ({
    ...
    const [mode, setMode] = useState<string>('filter_data')
    ...
    const initTableData = async (params: any) => {
        try {
            ...
            const data = await testWhiteList({
                form_view_id: configInfo.form_view_id,
                configs: configInfo.configs,
    ...
    return loading ? (
        ...
        >
            <Radio.Group
                ...
                size="middle"
            >
                <Radio.Button value="filter_data">
                    {__('被过滤的禁查数据')}
                </Radio.Button>
                <Radio.Button value="after_filter_data">
                    {__('允许查询的数据')}
                </Radio.Button>
            </Radio.Group>
            {/* <Watermark
