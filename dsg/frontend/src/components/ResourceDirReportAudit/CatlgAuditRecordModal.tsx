...
interface ICatlgAuditRecordModal {
    item: any
    onClose?: () => void
}
...
function CatlgAuditRecordModal({ item, onClose }: ICatlgAuditRecordModal) {
    const [loading, setLoading] = useState<boolean>(false)
    const [data, setData] = useState<ISSZDAuditRecordItem[]>([])

    useEffect(() => {
        if (item?.catalog_id) {
            getAuditRecord(item?.catalog_id)
        }
    }, [item])

    const getAuditRecord = async (id: string) => {
        try {
            setLoading(true)
            const res = await getSSZDAuditRecord(id)
            setData(res?.entries || [])
        } catch (error) {
            formatError(error)
        } finally {
...
export default CatlgAuditRecordModal
