...
const WorkOrderExcuteUrl = {
    [TaskType.RESEARCHREPORTWWORKORDER]:
        '/aggregationManage/investigationReport',
    [TaskType.DATACATALOGWWORKORDER]: '/dataService/dataContent',
    [TaskType.FRONTPROCESSORSWWORKORDER]: '/frontMachineMgt/apply',
}
...
const MyTask = () => {
    ...
    // 我的任务分类
    const [taskType, setTaskType] = useState<string>(MyTaskType.CREATED)
    ...
    useEffect(() => {
        ...
        const st = state !== null ? state : MyTaskType.CREATED
        setLoading(true)
        getExecutorsList()
        getTableData(
            {
        ...
        switch (op) {
            ...
            case OperateType.EXECUTE:
                if (
                    ...
                    ].includes(item?.task_type as any)
                ) {
                    window.open(
                        getActualUrl(
                            WorkOrderExcuteUrl[item?.task_type as string],
                            true,
                            LoginPlatform.drmb,
                        ),
                        '_blank',
            ...
            },
            {
                title: (
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                        {__('任务状态')}
                        {defaultStatusList.length !== 1 && (
                            <Dropdown
                                trigger={['click']}
                                dropdownRender={statusFilterDropdown}
                                destroyPopupOnHide
                                onOpenChange={(flag) =>
                                    setStatusSelectedOpen(flag)
                                }
                                open={statusSelectedOpen}
                            >
                                <FontIcon
                                    type={IconType.COLOREDICON}
                                    name="icon-a-shaixuanmianxing"
                                    style={{
                                        marginLeft: 4,
                                        fontSize: 12,
                                        cursor: 'pointer',
            ...
            },
            {
                title: (
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        <Dropdown
                            trigger={['click']}
                            dropdownRender={taskTypeFilterDropdown}
                            destroyPopupOnHide
                            onOpenChange={(flag) =>
                                setTaskTypeSelectedOpen(flag)
                            }
                            open={taskTypeSelectedOpen}
                        >
                            <FontIcon
                                type={IconType.COLOREDICON}
                                name="icon-a-shaixuanmianxing"
                ...
                key: 'task_type',
                ellipsis: true,
                render: (value) => <TaskTypeContent label={value} />,
            },
            {
                title: __('来源'),
                        ...
                        if (data) {
                            if (data.id) {
                                ...
                                getTableData({
                                    ...queryParams,
                                    offset:
                                        taskItems.length === data.ids.length
                                            ? queryParams.offset! - 1 || 1
                                            : queryParams.offset!,
                                })
                            }
