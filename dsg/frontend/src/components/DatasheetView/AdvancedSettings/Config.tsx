...
import { Button, message, Space, Spin, Table, Tooltip } from 'antd'
...
import { Empty, SearchInput, Watermark } from '@/ui'
...
import { execFilterRule, formatError, updateFilterRule } from '@/core'
    ...
    const saveFilterRule = async () => {
        try {
            ...
            const res = await execFilterRule(dataViewId, {
                ...
                // 在出现错误时，设置加载状态为false，设置错误状态，展开结果区域，并设置默认的大小分配。
                setLoading(false)
                setErrorStatus(err)
                setResultExpand(true)
                setDefaultUpSize([40, 60])
            ...
            // 更新过滤规则。
            await updateFilterRule(dataViewId, {
                filter_rule: trim(sqlScript),
            })
            // 显示保存成功的消息。
            message.success(__('保存成功'))
            // 调用确认回调，并传递修剪后的SQL脚本。
            onConfirm(trim(sqlScript))
        } catch (err) {
            // 处理任何捕获的错误。
            formatError(err)
        }
                        ...
                        <div className={styles.titleWrapper}>
                            ...
                            <Tooltip
                                title={
                                    <div>
                                        <span>
                                            {`示例：ds = date_format(CURRENT_DATE-INTERVAL '1' day,'%Y%m%d')`}
                                        </span>
                                        <Button
                                            type="link"
                                            onClick={() => {
