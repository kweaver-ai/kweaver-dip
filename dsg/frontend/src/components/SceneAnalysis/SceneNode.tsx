...
const SceneNodeComponent = (props: any) => {
    ...
    const handleRun = () => {
        ...
        const runCompareNode = callbackColl[6]()
        if (isCompare) {
            runCompareNode(node)
        } else {
            runNode(node)
    ...
    const getAddItems = (flag?: 'l' | 'r', item?: IFormula, index?: number) => {
        ...
        const filterFirstByFormulaNum = (num: number) => {
            // if (num >= 0) {
            //     res = res.filter((info) => ![FormulaType.WHERE].includes(info))
            // }
            if (num >= 1) {
        ...
        switch (item?.type) {
            ...
            default: {
                ...
                const sourceItem = graph.getEdges().find((edge) => {
                    return edge.getSourceCellId() === node.id
                })
                if (sourceItem) {
                    res = res.filter((info) => info !== FormulaType.COMPARE)
                }
                return res
            }
        }
    ...
    const handleDeleteOk = (item: IFormula, index: number) => {
        ...
        if (item.type === FormulaType.COMPARE) {
            const rightport = node.getPorts().find((port) => {
                return port?.group === 'out'
            })
            if (rightport && rightport.id) {
