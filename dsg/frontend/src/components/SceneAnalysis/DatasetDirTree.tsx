...
import { Checkbox, Dropdown, MenuProps, Radio } from 'antd'
import {
    Key,
    forwardRef,
    memo,
...
interface DataNode extends DataNodeFP {
...
const generateTreeData = (
    ...
    list
        .map((node) => {
            if (searchKey && node.name.includes(searchKey)) return null
            return {
                ...node,
                id: parent_id ? `${parent_id}_${node.id}` : node.id,
                isLeaf: !node.expand,
                children: node.children
                    ? generateTreeData(node.children, node.id)
                    : [],
            }
        })
        .filter(Boolean)
...
const ItemView = memo(
    ({
        ...
        const { name, uniform_catalog_code } = node
        ...
        return (
            <div ref={ref} className={styles['itemview-wrapper']} title={name}>
                ...
                <div className={styles['itemview-wrapper-nodename']}>
                    <div className={styles['itemview-wrapper-nodename-title']}>
                        {name}
                    </div>
                    {/* TODO:描述信息 */}
                    {uniform_catalog_code && (
                        <div
                            className={styles['itemview-wrapper-nodename-desc']}
                        >
                            {uniform_catalog_code}
                        </div>
                    )}
...
const SearchItem = memo(
    ({
        ...
        handleChangeCheckbox?: (...args: any[]) => void
    }) => {
        const { name, path, uniform_catalog_code } = item
        ...
        return (
            <div ref={ref} className={styles['search-item']}>
                {handleChangeCheckbox && uniform_catalog_code && (
                    <Checkbox
...
// 参数设置
const InitParams = { limit: 200, offset: 1 }
 ...
 * 我的数据集 - 废弃
 */
const DatasetDirTree: FC<Partial<IArchitectureDirTree>> = forwardRef(
    (props: any, ref) => {
        const {
            getSelectedNode,
            initParams,
        ...
        const [searchCheckedItems, setSearchCheckedItems] = useState<any[]>([])
        ...
        const getData = async (
            ...
            try {
                ...
                const responseData = await getDatasetTree(params)
                const res = responseData?.data || []
                let processRes: any = []
                if (isIncludeProcess && optType === DataOpt.Load) {
                    const processResData = await getViewsInDataset({
                        id: parent_id!,
                        keyword: '',
                        offset: 1,
                        limit: 0,
                    })
                    processRes = processResData?.data?.filter(
                ...
                switch (optType) {
                    case DataOpt.Init:
                        setData(generateTreeData(initData))
                        // if (
                        //     !isShowAll &&
                        //     initData?.length &&
                        //     !isIncludeProcess
                    ...
                    case DataOpt.Load:
                        setData((prev: DataNode[] | undefined) =>
                            generateTreeData([...prev!, ...res]),
                        )
                        break
                    case DataOpt.Search:
                        setData(generateTreeData(res, undefined, keyword))
                        setSearchResult(res)
        ...
        const handleSelect = (keys: Key[], info: any) => {
            ...
            } else if (node.uniform_catalog_code) {
                setCurrentNode(node)
            }
        }

        const handleCheck = (unuse, { checkedNodes }) => {
            const checkedItems = checkedNodes
                .filter(
                    // 区分是否是数据库表节点
                    (item) => item.uniform_catalog_code,
                )
                .map((item) => ({
                    ...item,
                    // 还原本来的id
...
const DatasetDirTreeContainer = forwardRef(
    (props: Partial<IArchitectureDirTree>, ref) => {
        return (
            <DirTreeProvider>
                <DatasetDirTree {...props} ref={ref} />
            </DirTreeProvider>
        )
    },
)

export default memo(DatasetDirTreeContainer)
