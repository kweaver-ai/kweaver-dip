                ...
                setSearchItems(
                    data?.filter((info) => {
                        ...
                        return (
                            ...
                            enName?.toLowerCase().includes(kw?.toLowerCase())
                        )
                    }) || [],
                )
            } else {
        ...
        const totalColumns: ColumnsType<any> = [
                ...
                title: (
                    <span>
                        ...
                        <span
                            hidden={
                                module !== ModuleType.SceneAnalysis ||
                                formulaItem?.type !== FormulaType.FORM
                            }
                            style={{
                                color:
                ...
                render: (value, record) => {
                    return (
                        <FieldItem
                            item={record}
                            fieldsData={fieldsData}
                            canDrag={record.beEditing ? false : !keyword}
            ...
            },
            {
                ...
                render: (value, record) =>
                    fieldLabel(
                        record?.data_type ||
                            fieldsData.data.find((a) => a.id === record.id)
                                ?.data_type,
                        record.originName,
                    ),
            },
            ...
            <div className={styles.fieldsDragTableWrap}>
                <div
                    className={
                        module === ModuleType.SceneAnalysis &&
                        formulaItem?.type === FormulaType.FORM
                            ? styles.configRightSubTitleCatalogWrap
                            : styles.configRightSubTitleWrap
                    }
                >
                    <span
                        className={styles.crt_tit}
                        hidden={
                            module === ModuleType.SceneAnalysis &&
                            formulaItem?.type === FormulaType.FORM
                        }
                    >
                        ...
                        <span className={styles.crt_desc}>
                            {formulaItem &&
                                (module === ModuleType.SceneAnalysis
                                    ? formulaInfo[formulaItem?.type].fieldsTip
                                    : formulaInfo[formulaItem?.type]
                                          .fieldsTip2 ||
                                      formulaInfo[formulaItem?.type].fieldsTip)}
                        </span>
                        <span
                        ...
                        <Table
                            ...
                            onRow={(record, index) => {
                                const attr = {
                                    index,
                                    canDrop: !keyword,
                                    canDrag: !record.beEditing && !keyword,
                                    moveRow,
                                }
