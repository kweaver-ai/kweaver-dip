...
import {
    cancelDataAggregationPlanAudit,
    changeDataAggregationPlanStatus,
    deleteDataAggregationPlan,
    formatError,
...
const PlanCollection = () => {
    ...
    const [auditApplyId, setAuditApplyId] = useState<string>('')
    ...
    const getDataList = async (params) => {
        try {
            setLoading(true)
            const obj = params
            const res = await getDataAggregationPlan(obj)
    ...
    const handleDelete = async (id: string) => {
        try {
            await deleteDataAggregationPlan(id)
    ...
    const handleCancelPlan = async (planId: string) => {
        try {
            await cancelDataAggregationPlanAudit(planId)
    ...
    const handleChangeStatus = async (planId: string, state: string) => {
        try {
            await changeDataAggregationPlanStatus(planId, state)
            refresh?.()
        } catch (error) {
            formatError(error)
        }
    ...
    return (
        <div className={styles['plan-collection']}>
