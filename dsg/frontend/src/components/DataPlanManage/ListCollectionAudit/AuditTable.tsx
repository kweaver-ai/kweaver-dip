...
import DetailModal from '../ListCollection/DetailModal'
...
const AuditTable = ({ target }: { target: AuditType }) => {
    ...
    const [searchCondition, setSearchCondition] = useState<any>({
        ...
        limit: 10,
        target,
        type: 'af-data-aggregation-inventory',
    })
    ...
    const columns: any = [
        ...
        },
        {
            ...
            render: (detail, record) => {
                ...
                const name = it?.name || record?.doc_names?.split(' ')?.[1]
                return (
                    <div
                        className={classnames(styles.line, styles.catlgName)}
                        title={name}
                        onClick={() => {
                            handleOperate(OperateType.PREVIEW, it)
                        }}
                    >
                        {name || '--'}
                    </div>
