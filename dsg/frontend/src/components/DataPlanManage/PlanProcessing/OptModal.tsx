...
import {
    checkNameDataProcessingPlan,
    createDataProcessingPlan,
    formatError,
    getDataProcessingPlanDetail,
    updateDataProcessingPlan,
} from '@/core'
...
const OptModal = ({ id, onClose }: any) => {
    ...
    const onFinish = async (values) => {
        if (needDeclaration.current && !values.plan_info) {
            setEditorInfoError(true)
            return
        }
        const { priority, date_range, responsible, ...rest } = values
        ...
        try {
            ...
            if (id) {
                await updateDataProcessingPlan(id, params)
            } else {
                await createDataProcessingPlan(params)
            }
                ...
                <div className={styles.body}>
                    ...
                    <div className={styles.footer}>
                        <Space size={16}>
                            ...
                            <Button onClick={handleSave}>{__('暂存')}</Button>
                            <Button
                                type="primary"
                                onClick={() => {
