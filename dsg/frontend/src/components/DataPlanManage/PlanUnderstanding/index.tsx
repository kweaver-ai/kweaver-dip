...
import {
    cancelDataComprehensionPlanAudit,
    changeDataComprehensionPlanStatus,
    deleteDataComprehensionPlan,
    formatError,
...
import ComprehensionOrderDetail from '../../WorkOrder/WorkOrderManage/DetailModal'
...
const RenderPlanOrder = ({
    ...
    return (
        <div>
            {items?.map((item, idx) => {
                return (
                    <div className={styles['sigle-line']}>
                        {idx + 1}、{item?.work_order_name}
...
const PlanUnderstanding = () => {
    ...
    const getDataList = async (params) => {
        try {
            setLoading(true)
            const obj = params
            const res = await getDataComprehensionPlan(obj)
    ...
    const handleDelete = async (id: string) => {
        try {
            await deleteDataComprehensionPlan(id)
    ...
    const handleCancelPlan = async (planId: string) => {
        try {
            await cancelDataComprehensionPlanAudit(planId)
    ...
    const handleChangeStatus = async (planId: string, state: string) => {
        try {
            await changeDataComprehensionPlanStatus(planId, state)
            refresh?.()
        } catch (error) {
            formatError(error)
        }
            ...
            render: (text: any[], record) => (
                <div
                    className={classnames(
                        styles.planOrder,
                        text?.length > 0 && styles.hasOrder,
                    )}
                    style={{ filter: `grayscale(${text?.length > 0 ? 0 : 1})` }}
                >
                    <FontIcon name="icon-gongdan" type={IconType.COLOREDICON} />
    ...
    return (
        <div className={styles['plan-understanding']}>
            ...
            {!!showOrder && (
                <ComprehensionOrderDetail
                    id={showOrder?.work_order_id}
                    type={OrderType.COMPREHENSION}
                    onClose={() => {
                        setShowOrder(undefined)
                    }}
                />
            )}
        </div>
...
export default PlanUnderstanding
