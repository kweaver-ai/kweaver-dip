...
import { searchFormInitData, initSearch, defaultMenu, sortMenus } from './const'
...
const LogTable: React.FC = (): React.ReactElement => {
    // 搜索条件
    ...
    const columns: any = useMemo(() => {
        const cols = [
            ...
            //     ellipsis: true,
            //     render: (value, record) => (
            //         <span title={record.call_department_path}>
            //             {value || '--'}
            //         </span>
            //     ),
            // },
    ...
    const handleExpansionStatus = (status: boolean) => {
        ...
        requestAnimationFrame(() => {
            setScrollY(status ? `calc(100vh - 420px)` : 'calc(100vh - 292px)')
        })
    }
    ...
    const handleExport = async () => {
        try {
            const res = await exportMonitorList(searchCondition)
            // 检查响应数据是否为有效的二进制数据
            if (res && typeof res === 'object' && res.byteLength > 0) {
                streamToFile(res, `接口服务日志.xlsx`)
    ...
    return (
        <div className={classnames(styles.serviceLogTable)}>
            <div className={styles.title}>{__('接口服务日志')}</div>
            {loading ? (
                renderLoader()
            ) : (
                <>
