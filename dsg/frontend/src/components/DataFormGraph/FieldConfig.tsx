...
import { GroupView2 } from '../AutoFormView/baseViewComponents'
...
import CodeRuleSelect from './CodeRuleSelect'
...
const FieldConfig = ({
            ...
            fields: [
                {
                    ...
                    field_id: `${currentData.uniqueId}`,
                    field_desc: currentData.description,
                },
            ],
        })
    }, [data])
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            ...
            if (currentData.name_en && !(model === 'view')) {
        ...
        try {
            ...
            if (fieldKey) {
                ...
                if (fieldKey[0] === 'is_standardization_required') {
                    setIsStandardizationRequired(
                        field.is_standardization_required,
                    )
                }
            }
        } catch (err) {
    ...
    const onFinish = (formData) => {
        ...
        if (isExistNode) {
            ...
            node.replaceData({
                ...node.data,
                items: [saveValues, ...node.data.items].map((item, index) => ({
                    ...item,
                    index: index + 1,
                })),
            })
        ...
        try {
            ...
            if (dataEleId) {
                ...
                const {
                    ...
                    code_table,
                    encoding_rule,
                } = DataFormStandardReplace(res.data)
    ...
    const formItemTemplate = {
        ...
        // ),
        value_range: (
            <Form.Item label={__('取值范围')} name="value_range">
                <Input placeholder={__('请输入取值范围')} />
            </Form.Item>
        ),
        code_table: (
            <Form.Item label={__('码表')} name="code_table">
                <CodeTableSelect
                    // value={currentData.code_table}
                    disabled={model === 'view'}
                    style={{ width: '100%' }}
                    onSelectDetails={(id, type) => {
                        setCodeTableVisible(true)
                        setDetailId(id)
                    }}
                />
        ...
        encoding_rule: (
            <Form.Item label={__('编码规则')} name="encoding_rule">
                <CodeRuleSelect
                    // value={currentData.encoding_rule}
                    onChange={(value) => {
                        form.setFieldValue('encoding_rule', value)
                    }}
                    disabled={model === 'view'}
                    style={{ width: '100%' }}
                    onSelectDetails={(id, type) => {
        ...
        name_detail: (
            <Row style={{ marginBottom: '16px' }}>
                <Col span={6} style={{ color: 'rgba(0, 0, 0, 0.45)' }}>
                    {__('字段业务名称：')}
                </Col>
                <Col span={18}>{currentData.name}</Col>
            </Row>
        ),
        ...
        name_en_detail: (
            <Row style={{ marginBottom: '16px' }}>
                ...
                <Col span={18}>{currentData.name_en}</Col>
            </Row>
        ),
        is_primary_key_detail: (
            <Row style={{ marginBottom: '16px' }}>
                ...
                <Col span={18}>
                    {currentData.is_primary_key ? __('是') : __('否')}
                </Col>
            </Row>
        ),
        is_required_detail: (
            <Row style={{ marginBottom: '16px' }}>
                ...
                <Col span={18}>
                    {currentData.is_required ? __('是') : __('否')}
                </Col>
            </Row>
        ),
        code_table_detail: (
    ...
    return (
        <div ref={containerBody}>
            <Drawer
                ...
                destroyOnClose
            >
                {/* {dataKind === FormTableKind.DATA_ORIGIN && (
                    <div className={styles.tipMessage}>
                        {__('表技术名称：默认带入库表技术名称，且不能改')}
                        {data.is_primary_key ? (
                            <div className={styles.isPrimaryKey}>
                                {__('主键')}
                            </div>
                        ) : null}
                    </div>
