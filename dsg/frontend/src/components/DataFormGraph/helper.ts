...
const checkRepeatName = (value, field, data) => {
    if (
        ...
        return Promise.reject(new Error(__('该字段业务名称已存在，请重新输入')))
    }
    return Promise.resolve()
}
 ...
 * @returns
 */
const checkRepeatNameEn = (value, field, data) => {
    if (
        data.find((item) => {
            // 数据源到入的业务表
            return item.id !== field.id && item.name_en === trim(value)
        })
    ) {
        return Promise.reject(new Error(__('该字段技术名称已存在，请重新输入')))
    }
    return Promise.resolve()
}
...
const getOriginDataRelation = (targetData, originData) => {
    return originData
        .filter((origin) => {
            return targetData.find((value) =>
                value.field_map?.source_field?.find(
                    (it) => it.field_id === origin.id,
                ),
            )
        })
 ...
 * @returns
 */
const removeTargetFormRelationByTable = (targetData, tableId) => {
    return targetData.map((item) => {
        return {
            ...item,
            field_map: {
                ...item?.field_map,
                source_field: item?.field_map?.source_field?.filter(
                    (it) => it.table_id !== tableId,
                ),
            },
        }
    })
...
const getYPosition = (
    ...
    index,
    model,
    targetTableKind = FormTableKind.DATA_ORIGIN,
) => {
    if (model === 'edit' && targetTableKind === FormTableKind.DATA_FUSION) {
        if (!site) {
        ...
        return {
            ...
            y:
                expandStatus === ExpandStatus.Expand
                    ? getYPosition(null, index, model, targetTableKind)
                    : 28,
        }
    }
    return {
        x: targetTableKind === FormTableKind.DATA_ORIGIN ? 0 : -4,
        y:
            expandStatus === ExpandStatus.Expand
                ? getYPosition(null, index, model, targetTableKind)
                : 28,
...
export const changeDataSheetFieldToBusinessForm = (fields) => {
...
const changeFieldIndexOrigin = (fields: any[], sortList: Array<string>) => {
    return fields.map((item) => ({
        ...item,
        index: sortList.findIndex((it) => item.id === it) + 1,
    }))
}
