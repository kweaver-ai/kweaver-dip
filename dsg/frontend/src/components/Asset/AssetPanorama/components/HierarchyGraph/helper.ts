...
import { IClassificationData, IHierarchy } from '@/core'
 ...
 * 组织架构基础数据单元
 */
export type INode = IClassificationData & { [key: string]: any }
...
export const NodeTypeText = {
    ...
    [AssetNodeType.ATTRIBUTE]: __('属性'),
}

const NodeSize = {
    [AssetNodeType.SUBJECTGROUP]: { width: 240, height: 72 },
    [AssetNodeType.SUBJECTGDOMAIN]: { width: 262, height: 46 },
    [AssetNodeType.BUSINESSOBJ]: { width: 234, height: 46 },
    [AssetNodeType.BUSINESSACT]: { width: 234, height: 46 },
    [AssetNodeType.LOGICENTITY]: { width: 206, height: 46 },
    [AssetNodeType.ATTRIBUTE]: { width: 178, height: 46 },
}
...
export const transformNodes = (node: INode) => {
    ...
    return {
        ...
        children:
            node.type === AssetNodeType.ATTRIBUTE // 显示到属性
                ? null
                : ![
                      AssetNodeType.BUSINESSACT,
                      AssetNodeType.BUSINESSOBJ,
