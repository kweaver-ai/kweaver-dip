...
function SubServiceAccess({
    ...
    useEffect(() => {
        if (subServices?.length) {
            // 默认展开加载第一项
            getAccess(subServices?.[0]?.id)
            setCurrentId(subServices?.[0]?.id)
        }
    }, [subServices])
    ...
    const getAccess = async (rid: string) => {
        try {
            setLoading(true)
            const ret: IPolicyInfo = await policyDetail(
                rid,
        ...
        <div className={styles['subview-access']}>
            {!subServices?.length ? (
                <div style={{ marginTop: '60px' }}>
                    <Empty iconSrc={dataEmpty} desc={__('暂无限定规则')} />
                </div>
            ) : (
                <Collapse
                    activeKey={currentId || ''}
...
export default SubServiceAccess
