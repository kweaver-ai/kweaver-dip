...
import { isEqualWith, debounce, trim } from 'lodash'
import {
    AssetTypeEnum,
    ISubView,
    PolicyActionEnum,
    PolicyEffectEnum,
    createIndicatorDimRules,
    formatError,
    policyCreate,
    policyDetail,
    policyUpdate,
    updateIndicatorDimRule,
} from '@/core'
import {
    ScopeType,
    SubViewOptType,
...
import ViewIndicatorDataByDim from './ViewIndicatorDataByDim'
...
const DimViewManage = ({
    ...
    const loadVisitor = async (item: ISubView) => {
        ...
        // 已有规则，从授权结果中获取
        const ret = await policyDetail(item?.id, AssetTypeEnum.Dim)
    ...
    const hasVisitAccess = useMemo(() => {
        // owner或者具有授权权限
        const hasReadAccess = (originData?.visitors || [])
        ...
        const scopeReadAccess = scopeSubject?.permissions?.some(
            (p) =>
                p.effect === PolicyEffectEnum.Allow &&
                p.action === PolicyActionEnum.Read,
        )
        // Owner | 当前用户具备读权限 | 具备整体读权限 | 授权范围具备读权限
        return isOwner || hasReadAccess || hasAllRead || scopeReadAccess
    }, [isOwner, originData, hasAllRead, userId, scopeSubject])
        ...
        try {
            ...
            if (ruleChange) {
                ...
                if (pageInfo?.isPass) {
                    ...
                    const subViewParams = {
                        ...JSON.parse(info?.detail || {}),
                        name: trim(info?.name || ''),
                        indicator_id: info.logic_view_id,
                        id: info.id,
                    }
                    const { id, ...params } = subViewParams
                    const { spec, metadata } = await updateIndicatorDimRule(
                        id,
                        params,
                    )
                    onOperation?.(SubViewOptType.Update, {
    ...
    const handleSure = async () => {
        ...
        try {
            const subViewParams = {
                ...JSON.parse(formData?.subView?.detail || {}),
                name: trim(formData?.subView?.name || ''),
                indicator_id: data.logic_view_id,
            }
            const subview = await createIndicatorDimRules({
                spec: subViewParams,
            })

            if (subjects?.length) {
                const policyParams = {
                    object_id: subview?.metadata?.id,
                    object_type: AssetTypeEnum.Dim,
                    subjects,
                }
    ...
    const handleChangedStatus = async () => {
        ...
        const isVisitorChange = !isEqualWith(curVisitor, originVisitor)
    ...
    const steps = useMemo(
        () => [
            ...
            {
                title: __('查看数据'),
                content: hasVisitAccess ? (
                    <ViewIndicatorDataByDim
                        // indicatorId={}
                        allFields={cols}
                        formData={formData}
                        formId={formId}
                        ...
                        openProbe={openProbe}
                        onDataChange={handleChangedStatus}
                        initialParams={viewAccessDataParams}
                        passParams={setViewAccessDataParams}
                    />
                ) : (
            ...
            },
            {
                title: __('添加访问者'),
                content: (
                    <VisitorProvider viewType={AssetTypeEnum.Dim}>
                        <VisitorCard
                            key="subview"
                            type={AssetTypeEnum.Dim}
    ...
    return (
        <div className={styles['process-panel']}>
            ...
            {current === 1 && (
                <div className={styles['process-panel-tips']}>
                    {__(
