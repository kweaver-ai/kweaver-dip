...
function ApiDetailContent(props: any) {
    ...
    const [commonTitleContent, setCommonTitleContent] =
        useState(commonTitleList)
    ...
    const getBasicTitle = (res: any) => {
        ...
        const basicConfig = basicCantainerContent.map((item) => {
            const newItemList = item.list.filter(
                (it) =>
                    !(
                        using === 1 &&
                        ['subject_domain_name', 'department'].includes(it.key)
                    ),
            )
            return {
                ...item,
            ...
            basicConfig.map((item, index) => {
                const newDataList =
                    res.service_type === 'service_generate'
                        ? item.list.filter(
                              (info) => !generateFilterData.includes(info.key),
                          )
                        : item.list.filter(
                              (info) => !createFilterData.includes(info.key),
                          )
                        ...
                        switch (key) {
                                    ...
                                    render: () => {
                                        return (
                                            <Radio.Group
                                                ...
                                                disabled
                                            >
                                                <Radio value="yes">同步</Radio>
                                                <Radio value="no">不同步</Radio>
                                            </Radio.Group>
                                        )
                                    },
                                }
                            ...
                            case 'file':
                                return {
                                    ...it,
                                    render: () => {
                                        if (res.file && res.file.file_name) {
                                            const suffix =
