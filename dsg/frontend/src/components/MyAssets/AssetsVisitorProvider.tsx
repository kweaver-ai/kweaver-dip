...
export const AssetsVisitorProvider = ({
    ...
    const [isDataOwner, setIsDataOwner] = useState<boolean | null>(null)
    ...
    const checkCurrentUserIsSystemManager = () => {
        try {
            // 获取当前用户的角色信息
            setIsAppDeveloper(
                checkPermission('manageIntegrationApplication') ?? false,
            )
            setIsSystem(checkPermission(allRoleList.TCSystemMgm) ?? false)
            setIsDataOwner(
                checkPermission('manageDataResourceAuthorization') ?? false,
            )
        } catch (err) {
            // 对错误进行格式化处理
            formatError(err)
    ...
    const values = useMemo(
        ...
        [selectedId, selectedType, isSystem, isDataOwner, isAppDeveloper],
    )
