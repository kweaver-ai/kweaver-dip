...
import { List, Pagination, Radio, Checkbox } from 'antd'
 ...
 * @returns
 */
function CatalogListMultiSelect(props: Partial<ICatalogList>) {
    ...
    const [checkItems, setCheckItems] = useState<IDataCatalogItem[]>(
        agentData?.config?.data_views || [],
    )
    ...
    const handleChangeCheckbox = (e, curr) => {
        const { id, business_name, technical_name, uniform_catalog_code } = curr
        const currItem = {
            id,
            view_name: `${business_name}（${technical_name}）`,
            view_desc: uniform_catalog_code,
            view_status: '1',
        }
        ...
        if (multiChecked) multiChecked(newCheckItems)
    }
    useEffect(() => {
        if (agentData?.config?.data_views && multiChecked) {
            multiChecked(agentData?.config?.data_views)
        }
    }, [])
...
export default memo(CatalogListMultiSelect)
