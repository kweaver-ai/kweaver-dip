...
func BindAndValid(c *gin.Context, v interface{}) (success bool, err error) {
...
func BindFormAndValid(c *gin.Context, v interface{}) (success bool, err error) {
	_, span := af_trace.StartInternalSpan(c.Request.Context())
	defer func() { af_trace.TelemetrySpanEnd(span, err) }()
...
func BindQueryAndValid(c *gin.Context, v interface{}) (success bool, err error) {
	_, span := af_trace.StartInternalSpan(c.Request.Context())
	defer func() { af_trace.TelemetrySpanEnd(span, err) }()
...
func BindUriAndValid(c *gin.Context, v interface{}) (success bool, err error) {
	_, span := af_trace.StartInternalSpan(c.Request.Context())
	defer func() { af_trace.TelemetrySpanEnd(span, err) }()
...
func BindJsonAndValid(c *gin.Context, v interface{}, noDictValidate ...bool) (success bool, err error) {
	_, span := af_trace.StartInternalSpan(c.Request.Context())
	defer func() { af_trace.TelemetrySpanEnd(span, err) }()
	...
	if !containsBool(noDictValidate, true) { //不校验字典本身
		//校验数据字典
		checkType, err := validation.CheckDictTypeKey(c.Request.Context(), v)
		if err != nil {
			var validErrors ValidErrors
			validErrors = append(validErrors, &ValidError{
				Key:     "dict_check_databases_error",
				Message: "校验字典查询参数查询数据库异常",
			})
			return false, validErrors
		}
		if checkType != "" && len(checkType) > 0 {
			var validErrors ValidErrors
			validErrors = append(validErrors, &ValidError{
				Key:     "dict_check_type_key_error",
				Message: "校验字典查询参数输入值不合法，不合法字段包括" + checkType,
			})
			return false, validErrors
		}
	}
...
func containsBool(s []bool, e bool) bool {
	for _, a := range s {
		if a == e {
			return true
		}
